...
def load_chunks(path: str = None) -> List[Dict[str, Any]]:
    ...
    try:
        ...
        if validation_result["invalid_chunks"] > 0:
            print(f"⚠️ Warning: {validation_result['invalid_chunks']} chunks failed validation")
...
def save_sample(chunks: List[Dict[str, Any]], sample_path: str, sample_size: int = 30) -> None:
    ...
    print(f"🧪 Sample of {len(sample)} chunks saved to: {sample_path}")
...
def save_flagged_json(chunks: List[Dict[str, Any]], flagged_json_path: str) -> None:
    ...
    print(f"🚫 {len(flagged)} flagged chunks saved to: {flagged_json_path}")
...
def export_flagged_csv(chunks: List[Dict[str, Any]], csv_path: str, csv_export_limit: int = 50) -> None:
    ...
    flagged = [c for c in chunks if c.get("omit_flag")]
    if not flagged:
        print("❌ No flagged chunks to export")
        return
    ...
    print(f"📊 {len(flagged[:csv_export_limit])} flagged chunks exported to CSV: {csv_path}")
...
def print_semantic_summary(histogram_data: dict) -> None:
    ...
    if histogram_data['total_chunks'] > 0:
        ...
        if other_percentage > 30:
            print(f"\n⚠️  WARNING: High 'other' percentage ({other_percentage:.1f}%) - semantic tagging may need improvement")
        else:
            print(f"\n✅ Good semantic distribution - 'other' percentage: {other_percentage:.1f}%")
    else:
        print(f"\n⚠️  No chunks to analyze")
